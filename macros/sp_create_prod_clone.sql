...
WHILE (existing_schemas.next()) { var schema = existing_schemas.getColumnValue(1);
snowflake.execute(
        { sqlText: `GRANT OWNERSHIP ON SCHEMA ${DESTINATION_DB_NAME}.${schema} TO ROLE ${ROLE_NAME} COPY CURRENT GRANTS;` }
    );
snowflake.execute(
        { sqlText: `REVOKE OWNERSHIP ON FUTURE FUNCTIONS IN SCHEMA ${DESTINATION_DB_NAME}.${schema} FROM ROLE DBT_CLOUD_BSC` }
    );
snowflake.execute(
        { sqlText: `REVOKE OWNERSHIP ON FUTURE PROCEDURES IN SCHEMA ${DESTINATION_DB_NAME}.${schema} FROM ROLE DBT_CLOUD_BSC` }
    );
snowflake.execute(
        { sqlText: `REVOKE OWNERSHIP ON FUTURE TABLES IN SCHEMA ${DESTINATION_DB_NAME}.${schema} FROM ROLE DBT_CLOUD_BSC` }
    );
snowflake.execute(
        { sqlText: `REVOKE OWNERSHIP ON FUTURE VIEWS IN SCHEMA ${DESTINATION_DB_NAME}.${schema} FROM ROLE DBT_CLOUD_BSC` }
    );
snowflake.execute(
...
WHILE (existing_tables.next()) { var schema = existing_tables.getColumnValue(1);
...
WHILE (existing_functions.next()) { var schema = existing_functions.getColumnValue(1);
...
WHILE (existing_procedures.next()) { var schema = existing_procedures.getColumnValue(1);
